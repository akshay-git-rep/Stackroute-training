...
from profiles_api.models import Book, Post, Laptop  # Import the status module
...
class UserLaptopApiView(APIView):
    ...
    def get(self, request):
        ...
        return Response({'Laptops': list(laptops)})

    def post(self, request):
        """post method to add new Laptop"""

        serializer = self.serializer_class(data=request.data)

        if serializer.is_valid():
            modelname = serializer.validated_data.get('modelname')
            price = serializer.validated_data.get('price')
            ram = serializer.validated_data.get('ram')
            description = serializer.validated_data.get('description')
            laptop = Laptop.objects.create(modelname=modelname, ram=ram, price=price, description=description)
            message = f'Laptop Added {laptop}'
            return Response({'message': message})
        else:
            return Response(
                serializer.errors,
...
class UserLaptopViewSet(viewsets.ModelViewSet):
    """Test API ViewSet"""
    serializer_class = serializers.LaptopModelSerializer
